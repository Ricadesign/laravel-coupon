    ...
    public function searchCoupon(Request $request) {
        ...
        if ($coupon && $coupon->isValid) {
            if(config('laravel-coupon.unique_email')){
                ...
                if($redemption){
                    return response()->json(['error' => 'Coupon already redeemed'], 401);
                }
            }
            ...
            if(config('laravel-coupon.coupon_for_product')){
                ...
                if($typeCoupon){
                    if(sizeof($coupon->validProductType($typeProduct, $typeCoupon)) > 0) return $coupon->only(['code_name', 'requirements', 'type', 'value_discount', 'req_qty', 'products_type']); 
